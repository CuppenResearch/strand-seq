...
void printUsage() {
    ...
    stderr.writeln("         -i, --input (required)");
    stderr.writeln("                    folder with the input bam files");    
    ...
    stderr.writeln("                    minimal percentage of 'passed' cells in the input supporting the switchpoint (DEFAULT: 20%)");
    stderr.writeln("         -q, --MQ");
...
int main(string[] args) {
    getopt(args, std.getopt.config.caseSensitive, std.getopt.config.passThrough,
      ...
      "count|c", &input_count_perc,
      "MQ|q", &MQ,
      "input|i", &inputFolder,
    );
  
  // die and show usage if help or input is empty
  if (help || inputFolder == null) {
    printUsage();
    return 0;
  }
  
  // get name of the sample
  inputName = get_output_name(inputFolder);
  
  // get median distributions
  double[] bin_size_median_distribution_input = bam_looping(inputFolder);
  
  double bin_size_median_input_mean = mean(bin_size_median_distribution_input); // calculate the mean of the median distribution
  double merge_distance = bin_size_median_input_mean/distance_factor; // calculate the maximal merge distance between two switchpoints
  merge_switch_points(inputFolder, merge_distance); // call denovo switch points clusters
...
double[] qc_analysis(DirEntry[] bamFiles, double[] total_reads_distribution, double[] FR_bins_ratio_distribution, double[] bin_size_median_distribution, double[] number_of_bins_distribution, string folder) {
  ...
  if (outName == inputName) { input_count = to!double(passed_cells)/100*to!double(input_count_perc); } 
  return(bin_size_median_distribution2); // return meadian distribution of only passed QC files
}

// function to merge switch points
void merge_switch_points(string inputFolder, double merge_distance) {
  
  ulong[DirEntry][string][ulong][string] switch_points; // initialize switch points variable  
  
  // initialize output file name
  string outputName = "switch_points_i"~inputName~"_r"~to!string(reads_per_bin)~"_d"~to!string(distance_factor)~"_c"~to!string(input_count_perc)~"_q"~to!string(MQ)~".txt";
  File output = File(outputName,"w"); // open output file
  
  // write header to the output file
  output.writeln("##INPUT: ",inputName);
  output.writeln("##READS: ",reads_per_bin);
  output.writeln("##DISTANCE FACTOR: ",distance_factor);
  output.writeln("##INPUT COUNT PERCENTAGE: ",input_count_perc);
  output.writeln("##MERGE_DISTANCE: ",merge_distance);  
  output.writeln("##INPUT COUNT: ",to!ulong(input_count));  
  output.writeln("#CHR\tSTART\tEND\t",inputName,"\tFILTER\tREASON");
    
  string name = get_output_name(inputFolder); // get output name of the sample
  auto pointsFiles = array(filter!`endsWith(a.name,".points")`(dirEntries(inputFolder,SpanMode.depth))); // get all *.points files
  foreach( pointsFile ; pointsFiles ) { // loop through all switch points files      
    if (pointsFile in qc_fail[inputFolder]) { continue; } // skip qc failed cells
  ...
  foreach( chr ; switch_points.keys.sort ) { // loop through all switch points in all files of all samples
	...
	foreach( file ; switch_points[chr][pos][sample].keys ) { // loop through each file of the sample containing the switch point position
	  if ((pos-prev_pos) < merge_distance || prev_pos == -1) { // if the distance between two switch points is less then 'variable', except the first switch point position
	    ...
	    ulong input_switch_points = 0;
	    
	    // increase the number of switch point in the switch point region found in each sample
	    if (inputName in samples) { input_switch_points = samples[inputName].keys.length; }
	    
	    // if the percentage of switch points in the switch point region found in the input is less then 'variable', report as failed
	    if (input_switch_points < input_count) {
	      output.writeln(chr,"\t",switch_point_region[0],"\t",switch_point_region[$-1],"\t",input_switch_points,"\t","FAILED","\t","INPUT COUNT");
	    // report input denovo switch points
	    } else {
	      output.writeln(chr,"\t",switch_point_region[0],"\t",switch_point_region[$-1],"\t",input_switch_points,"\t","PASS","\t","-");
	    }
